...
SarProjectXYZ::SarProjectXYZ(ros::NodeHandle &nh):
    nh_(nh)
{
...
//    }
    ...
    if (robot_connected_)
    {
        ...
        sub_pose_ = nh_.subscribe<geometry_msgs::PoseStamped>("/" + robot_type_ +"_driver/out/tool_pose", 1, &SarProjectXYZ::get_current_pose, this);
    }
...
void SarProjectXYZ::get_current_state(const sensor_msgs::JointStateConstPtr &msg)
{
    boost::mutex::scoped_lock lock(mutex_state_);
    current_state_ = *msg;
}

void SarProjectXYZ::get_current_pose(const geometry_msgs::PoseStampedConstPtr &msg)
{
...
void SarProjectXYZ::clear_workscene()
{
    // remove table
    co_.id = "table";
...
void SarProjectXYZ::build_workscene()
{
...
void SarProjectXYZ::clear_obstacle()
{
    co_.id = "pole";
    co_.operation = moveit_msgs::CollisionObject::REMOVE;
    pub_co_.publish(co_);
...
void SarProjectXYZ::add_obstacle()
{
    clear_obstacle();
...
void SarProjectXYZ::add_complex_obstacle()
{
    clear_obstacle();
...
void SarProjectXYZ::add_attached_obstacle()
{
...
void SarProjectXYZ::check_collision()
{
...
void SarProjectXYZ::define_cartesian_pose()
{
    tf::Quaternion q;
    ...
    // define pub pose
    pub_pose_xyz.resize(3);
    pub_pose_xyz = {-0.14, -0.01, 0.20};
    pub_pose_.header.frame_id = "root";
    pub_pose_.header.stamp = ros::Time::now();
    pub_pose_.pose.position.x = pub_pose_xyz[0];
    pub_pose_.pose.position.y = pub_pose_xyz[1];
    pub_pose_.pose.position.z = pub_pose_xyz[2];

    q = EulerZYZ_to_Quaternion(-M_PI/4, M_PI/2, M_PI);
    pub_pose_.pose.orientation.x = q.x();
    pub_pose_.pose.orientation.y = q.y();
    pub_pose_.pose.orientation.z = q.z();
    pub_pose_.pose.orientation.w = q.w();
}

void SarProjectXYZ::define_joint_values()
{
 ...
 */
geometry_msgs::PoseStamped SarProjectXYZ::generate_gripper_align_pose(geometry_msgs::PoseStamped targetpose_msg, double dist, double azimuth, double polar, double rot_gripper_z)
{
...
void SarProjectXYZ::setup_constrain(geometry_msgs::Pose target_pose, bool orientation, bool position)
{
...
void SarProjectXYZ::check_constrain()
{
...
void SarProjectXYZ::evaluate_plan(moveit::planning_interface::MoveGroup &group)
{
    bool replan = true;
    int count = 0;
...
void SarProjectXYZ::getInvK(geometry_msgs::Pose &eef_pose, std::vector<double> &joint_value)
{
...
{
    ros::init(argc, argv, "sar_project_demo");
    ros::NodeHandle node;
    ros::AsyncSpinner spinner(1);
    spinner.start();

    kinova::SarProjectXYZ sar_place(node);

    ros::spin();
    return 0;
}

bool SarProjectXYZ::sar_move()
{
    clear_workscene();
    ros::WallDuration(1.0).sleep();
    // build_workscene();
...
void SarProjectXYZ::get_params_n_print(){
    ros::param::get("/pub_joint_param", pub_joint_param_cpp_degree);
...
void SarProjectXYZ::sar_evaluate_plan(moveit::planning_interface::MoveGroup &group)
{
    bool replan = true;
    int count = 0;
...
void SarProjectXYZ::sar_cal_pose(std::vector<double> &pose_xyz){
    ...
    pub_pose_.header.frame_id = "root";
    pub_pose_.header.stamp = ros::Time::now();
    pub_pose_.pose.position.x = pose_xyz[0];
    pub_pose_.pose.position.y = pose_xyz[1];
    pub_pose_.pose.position.z = pose_xyz[2];
